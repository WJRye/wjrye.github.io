...
| 类型 |  实现目标 |
| --- | --- |
| 素材维度| 根据自研视频编辑器引擎支持素材类型进行灰度 |
| 原子能力为度 |根据自研视频编辑器引擎支持的原子能力进行灰度  |
...
输出项目依赖的三方 aar 信息，目的是给自研SDK参考，避免依赖冲突，避免构建问题。
...
输出项目依赖的三方 so 信息，目的是给自研SDK参考，避免依赖冲突，避免构建问题。
...
1. 通过这些 api 输出 SDK 原子能力，让自研SDK实现这些原子能力，并提供与当前SDK具有相同功能的 api 。
...
Factory[SD K对象工厂] --返回--> SDKObject --返回--> Adapter


Factory --创建--> ThirdObject[三方 SDK 对象]
Factory --创建--> SlefObject[自研 SDK 对象]
...
SDKEnter==获取==>SDKManager
SDKManager==返回==>SDKEnter
end
```
