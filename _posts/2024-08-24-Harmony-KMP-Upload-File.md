...
2. 上传文件涉及访问网络，存储数据，而网络库和数据库在 KMP 中，要同时支持 Android, iOS, Harmony 技术上不可行。
...
上传文件的技术目标是上传成功率（包括：文件大小，耗时时间，业务场景等），需要统计数据有：
1. 上传开始
2. 上传成功
3. 上传失败
4. 上传取消

上传成功率 = 上传成功 /（上传开始-上传取消）。
...
最后，统计数据最为重要，这关系着上传文件库的质量，开发完线上运行结果是好是坏就看这个性能指标数据了。
